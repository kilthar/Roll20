...
AlterScript.Process = function (msg) {
	...
	var Bar2Key = "2";
	var Bar3Key = "3";
	var Bar4Key = "4";  // Used for Non Lethal Damage
	...
	var Bar4Gain = ["recovers", "hit point", "non-lethal damage"]
	var Bar4Loss = ["takes", "damage", "non-lethal damage"]
...
//	var CurrentValueNLD = parseInt(Target.get("non-lethal-damage"));

    // Check to see if a valid token is selected, exit if not valid
	if (CurrentValue != 0) {
	if (!CurrentValue) {
		sendChat(who,"That is not a valid token.");
		return;
	}	
	}
	...
	//	var TargetName = (Target.get("name") == "" || Target.get("showplayers_name") == false && !ALERT_GM) ? "NPC" : Target.get("name");
		...
		if (Operand1 != "-") {
			...
			if (Bar == 4 && CurrentValue - AlterValue < 0) AlterValue = CurrentValue;

			var AlertGain = "" +
                "<div style='display: block; margin-left: -7px; margin-right: 2px; padding: 2px 0px;'>" +
                    "<div style='position: relative; border: 1px solid #000; border-radius: 5px; background-color: " + ((!ALT_COLORS) ? "#BF5700" : "#004400") + "; background-image: linear-gradient(rgba(255, 255, 255, .3), rgba(255, 255, 255, 0)); margin-right: -2px; padding: 12px 5px 15px 50px;'>" +
                        "<div style='position: absolute; top: 0px; left: 5px; height: 40px; width: 40px;'>" + 
			...
			if (Bar != 4) {
				...
				// Target.set("bar" + Bar + "_value", CurrentValue -= AlterValue);
				Target.set("non-lethal-damage", CurrentValue -= AlterValue);
				// setAttr(R20character,"non-lethal-damage",characterObj.personal._skin);
			}
		} else {
			// Subtract from bar...
			var AlertLoss = "" +
                "<div style='display: block; margin-left: -7px; margin-right: 2px; padding: 2px 0px;'>" +
                    "<div style='position: relative; border: 1px solid #000; border-radius: 5px; background-color: " + ((!ALT_COLORS) ? "#BF5700" : "#C60000") + "; background-image: linear-gradient(rgba(255, 255, 255, .3), rgba(255, 255, 255, 0)); margin-right: -2px; padding: 12px 5px 15px 50px;'>" +
                        "<div style='position: absolute; top: 0px; left: 5px; height: 40px; width: 40px;'>" + 
                            "<img src='" + Target.get("imgsrc") + "' style='height: 40px; width: 40px;'></img>" + 
                        "</div>" +
                        "<div style='font-family: Candal; font-size: 13px; line-height: 15px; color: #FFF; font-weight: normal; text-align: center; text-shadow: -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000;'>" + 

...
on("chat:message", function(msg) {
    ...
    if(msg.type == "api" && msg.content.indexOf("!range") != -1)
    {
        ...
        // DnD 4.0 Distance Calculation
            ...
            var zDist = Math.abs((token1.get("bar2_value")-token2.get("bar2_value"))/curScale);//gridSize;
        ...
        // DnD 3.5 / Pathfinder Distance Calculation
			...
			if (zDist >= 70)
				{
				zDist = zDist - 70;// based on medium size tokens reduce effective height difference by 1 unit as token is a cube
				}
			if (useHeight != true) zDist = 0;
            var dist = 0;
			dist = Math.floor(1.5 * Math.min(lDist, tDist) + Math.abs(lDist - tDist));
